...
urlpatterns = [
    ...
    path("<int:pk>/update", login_required(UpdateView.as_view(model=Transaction,
         template_name="balance/update.html", fields=["name", "purpose", "transaction_type", "amount", "due_date", "repeat_pattern", "website", "email", "telephone", "end_date",], success_url=reverse_lazy("balance"))), name="update"),
    path("<int:pk>/delete", login_required(DeleteView.as_view(model=Transaction,
         template_name="balance/delete.html", success_url=reverse_lazy("balance"))), name="delete"),
]
